    ...
    for(Long64_t ievt = 0; ievt < numEntries; ++ievt) {
        ...
        chain->GetEntry(ievt);
				...
				for(size_t iB = 0; iB < BToKsEE_mass.GetSize(); ++iB) {
					...
					counter6++;
					if(BToKsEE_fit_mass[iB] < 4.8 || BToKsEE_fit_mass[iB] > 5.6) continue;
					if(BToKsEE_fit_mass[iB] < 5.4 && BToKsEE_fit_mass[iB] > 5.0) continue;
					counter7++;
...
void select_Data_131_quadruplets(TString inputList, TString name, int ifile, int mode) {
